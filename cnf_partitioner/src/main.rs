...
use crate::cnf::{condition_instance, find_assignment, get_cut_variables, serialize_cnf};
...
struct Args {
    ...
    /// The input is a directory of CNF DIMACS files that should be all evaluated.
    #[arg(short, long, env)]
    recursive: bool,
    ...
    /// Whether to preprocess the input CNF before partitioning it.
    #[arg(short, long, env, default_value_t = true)]
    preprocess: bool,

    /// Maximum allowed time in seconds for a d4 run on split CNF parts
    #[arg(short, long, env)]
    timeout: Option<u64>,

    /// Path where to write the optionally preprocessed CNF to.
    #[arg(long, env)]
...
fn main() -> io::Result<()> {
    ...
    // Preprocess the input in case the flag is set.
    let input = if args.preprocess {
        ...
        // Otherwise read the input CNF directly.
        fs::read_to_string(args.input.clone())
    };
    ...
    for partitioner in &partitioners {
        ...
        let cnfs = partition.split_cnf(&cnf);
        ...
        let (original_time, original_count) = {
